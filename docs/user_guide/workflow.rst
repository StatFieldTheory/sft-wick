...
.. list-table::
   ...
   * -
     ...
     - Slow; split at kinks like ``gauss_legendre``; a callable coupling is evaluated once per quadrature point and symbol, so cost grows with the adaptive subdivision
   * -
     - ``qmc`` / ``qmc_scalar``
