...
3. **Symbolic LaTeX rendering** of an equal-time vertex still emits
   ...
   This is cosmetic: the numerical integration uses the
...
6. `Vertex(local=True, equal_time=True)` raises `ValueError`, guarding
   against a flag on a local vertex leaving the spurious
   `t_max^(m-1)` factor in place undetected
...
13. Jacobian ratio: equal_time integrates to `span` and full integrates
    to `span^m`, which exposes the `(t_max)^(m-1)` correction (GL path)
...
17. End-to-end: `System(equal_time NonLocalVertex).expand(...)`
    ...
    (spec through to an integration-ready DiagramTerm)
